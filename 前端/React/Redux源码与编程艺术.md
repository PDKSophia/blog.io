...
这就是我们为啥在 `react` 项目中，总能看到，在根 App 组件都有这么个玩意了。
...
> 在 react 中，我们从 `this.props.xxx` 中读取数据。为什么我们可以得到最新的实例？其实不是因为 props 改变了，在 React 中 Props 是不可变(immutable)的，他们永远不会改变。然而，this 是，而且永远是，可变(mutable)的。

> 这就是 react 类组件 this 存在的意义。React 本身会随着时间的推移而改变，以便你可以在渲染方法以及生命周期方法中得到最新的实例。
...
> 别问，问就是我也有自己公众号，不过我不会公开，公众号是 17 年开通的，起初是自己想写一些身边朋友的故事（比较非主流）后来由于自己写的博客文章被偷到一些公众号上，为了维护原创，你懂的～
